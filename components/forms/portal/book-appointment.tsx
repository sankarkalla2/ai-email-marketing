...
import { APPOINTMENT_TIME_SLOTS } from "@/constants/appointmen-slot";
  ...
  return (
    <div className="flex flex-col gap-5 justify-center">
      <div className="flex justify-center">
        <h2 className="text-4xl font-bold mb-5">Meeting</h2>
      </div>
          ...
          {APPOINTMENT_TIME_SLOTS.map((slot, key) => (
            <Label htmlFor={`slot-${key}`} key={key}>
              <Card
                ...
                className={cn(
                  currentSlot == slot.slot ? "bg-orange-400" : "bg-orange-200",
                  "px-10 py-4",
                  bookings &&
                    bookings.some(
                      (booking) =>
