...
export type UserLoginSchemaProps = {
  email: string;
  password: string;
};
...
export const UserLoginSchema: ZodType<UserLoginSchemaProps> = z.object({
...
export const changePasswordSchema: ZodType<ChangePasswordProps> = z
  .object({
    password: z
      .string()
      .min(8, { message: "Your password must be alteast  8 character long" })
      .max(64, { message: "Your password can't be longer than 64 characters" })
      .refine(
        (value) => /^[a-zA-Z0-9_.-]*$/.test(value ?? ""),
        "password should contain only alphabets and numbers"
      ),
